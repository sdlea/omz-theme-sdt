PROMPT="%(?:%{$fg_bold[green]%}%n%1{ >>%}:%{$fg_bold[red]%}%n%1{ x>%}) %{$fg[cyan]%}%2~%{$reset_color%}%3{ %}"
...
ZSH_THEME_GIT_PROMPT_SUFFIX="%{$reset_color%}%1{ %}"
ZSH_THEME_GIT_PROMPT_DIRTY="%{$fg[yellow]%} !%{$fg[blue]%}%1{)%}"
ZSH_THEME_GIT_PROMPT_CLEAN="%{$fg[blue]%}%3{ .)%}"
...
function git_prompt_info () {
	...
	dtls=$(__concat " " \
		$(git_commits_ahead) \
		$(git_commits_behind) \
		$(__concat "," $(printf "%s" "$(__git_prompt_git status --porcelain=v1)" | grep "^[AM]" | wc -l)) \
